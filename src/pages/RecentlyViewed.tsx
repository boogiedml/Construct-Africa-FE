...
type RecentlyViewedCategory = 'projects' | 'companies' | 'main_news' | 'tenders';

const RecentlyViewed = () => {
  const navigate = useNavigate();
  const [activeView, setActiveView] = useState('table');
  const [activeCategory, setActiveCategory] = useState<RecentlyViewedCategory>('projects');
  ...
  const { data: recentlyViewedData, isLoading, isError, refetch } = useGetRecentViewsQuery(queryParams);
  ...
  const getGroupApiKey = (category: RecentlyViewedCategory): 'projects' | 'companies' | 'news' | 'tenders' => {
    return category === 'main_news' ? 'news' : category;
  };

  // Extract and flatten items - Filter by active category only
  const displayItems = useMemo(() => {
    console.log(recentlyViewedData?.recent_views);
    ...
    if (!recentlyViewedData?.recent_views || !recentlyViewedData.recent_views[apiCategoryKey]) {
      return [];
    }

    // Only process items for the current active category
    const categoryData = recentlyViewedData.recent_views[apiCategoryKey];
    ...
    return data;
  }, [recentlyViewedData, activeCategory]);


  // Counts uses 'main_news' key, so we can use activeCategory directly
  const itemCount = recentlyViewedData?.counts?.[activeCategory] || 0;
    ...
    <div className="min-h-screen mx-auto py-5 md:py-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-[#535862]">Showing {itemCount} recently viewed {activeCategory}</p>
        </div>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Tabs
          ...
          onTabChange={(tab) => {
            setActiveCategory(tab as RecentlyViewedCategory);
            setCurrentPage(1);
          }}
        />
      </div>
